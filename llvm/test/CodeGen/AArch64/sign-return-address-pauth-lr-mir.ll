...
define i32 @leaf_sign_all_v83(i32 %x) "branch-protection-pauth-lr" "sign-return-address"="all" "target-features"="+v8.3a" {
  ...
  ; CHECK-MIR-NEXT:   $x16 = frame-destroy ADDXri $x16, target-flags(aarch64-pageoff, aarch64-nc) <mcsymbol >, 0
  ; CHECK-MIR-NEXT:   frame-destroy PACM
  ; CHECK-MIR-NEXT:   frame-destroy RETAA implicit $lr, implicit $sp, implicit-def $lr, implicit killed $lr, implicit $sp
  ;
...
define fastcc void @spill_lr_and_tail_call(i64 %x) "branch-protection-pauth-lr" "sign-return-address"="all" {
  ...
  ; CHECK-MIR-NEXT:   frame-setup CFI_INSTRUCTION def_cfa_offset 16
  ; CHECK-MIR-NEXT:   frame-setup CFI_INSTRUCTION offset $w30, -16
  ; CHECK-MIR-NEXT:   INLINEASM &"mov x30, $0", 1 /* sideeffect attdialect */, 3866633 /* reguse:GPR64common */, renamable $x0, 12 /* clobber */, implicit-def dead early-clobber $lr
  ; CHECK-MIR-NEXT:   early-clobber $sp, $lr = frame-destroy LDRXpost $sp, 16 :: (load (s64) from %stack.0)
  ; CHECK-MIR-NEXT:   $x16 = frame-destroy ADRP target-flags(aarch64-page) <mcsymbol >
  ; CHECK-MIR-NEXT:   $x16 = frame-destroy ADDXri $x16, target-flags(aarch64-pageoff, aarch64-nc) <mcsymbol >, 0
  ; CHECK-MIR-NEXT:   frame-destroy PACM
...
define i32 @leaf_sign_all_b_key(i32 %x) "branch-protection-pauth-lr" "sign-return-address"="all" "sign-return-address-key"="b_key" {
  ...
  ; V83A-MIR-NEXT:   $x16 = frame-destroy ADDXri $x16, target-flags(aarch64-pageoff, aarch64-nc) <mcsymbol >, 0
  ; V83A-MIR-NEXT:   frame-destroy PACM
  ; V83A-MIR-NEXT:   frame-destroy RETAB implicit $lr, implicit $sp, implicit-def $lr, implicit killed $lr, implicit $sp
  ;
...
define i32 @leaf_sign_all_v83_b_key(i32 %x) "branch-protection-pauth-lr" "sign-return-address"="all" "target-features"="+v8.3a" "sign-return-address-key"="b_key" {
  ...
  ; CHECK-MIR-NEXT:   $x16 = frame-destroy ADDXri $x16, target-flags(aarch64-pageoff, aarch64-nc) <mcsymbol >, 0
  ; CHECK-MIR-NEXT:   frame-destroy PACM
  ; CHECK-MIR-NEXT:   frame-destroy RETAB implicit $lr, implicit $sp, implicit-def $lr, implicit killed $lr, implicit $sp
  ;
