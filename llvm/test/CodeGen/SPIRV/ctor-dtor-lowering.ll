...
; RUN: llc -verify-machineinstrs -mtriple=spirv64-intel-unknown --spirv-ext=+SPV_INTEL_function_pointers %s -o - | FileCheck %s
; Fix when spir-val supports the SPV_INTEL_function_pointers extension:
; FIXME: %if spirv-tools %{ llc -verify-machineinstrs -O0 -mtriple=spirv64-intel-unknown --spirv-ext=+SPV_INTEL_function_pointers %s -o - -filetype=obj | spirv-val %}
