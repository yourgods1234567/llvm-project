...
define <vscale x 4 x i1> @vselect_both_neverzero(<vscale x 4 x i1> %cond, <vscale x 4 x float> nofpclass(zero) %a, <vscale x 4 x float> nofpclass(zero) %b) {
...
; CHECK-NEXT:    ret
  %sel = select <vscale x 4 x i1> %cond, <vscale x 4 x float> %a, <vscale x 4 x float> %b
  %class = call <vscale x 4 x i1> @llvm.is.fpclass.nxv4f32(<vscale x 4 x float> %sel, i32 96)  ; 0x60 = "zero"
  ret <vscale x 4 x i1> %class
}
