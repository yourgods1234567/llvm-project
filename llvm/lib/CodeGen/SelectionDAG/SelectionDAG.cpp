...
SDValue SelectionDAG::FoldSetCC(EVT VT, SDValue N1, SDValue N2,
  ...
  if (ConstantSDNode *N2C = dyn_cast<ConstantSDNode>(N2)) {
    ...
    // Fold setugt(X, 0) -> setne(X, 0): 0 is the minimum unsigned value,
    // so X > 0 iff X != 0. This must be in FoldSetCC (not just SimplifySetCC)
    // so the fold fires at construction time before expanded types like i128
    // undergo multi-word comparison lowering.
