...
void MorphOS::addClangTargetOptions(
    const llvm::opt::ArgList &DriverArgs, llvm::opt::ArgStringList &CC1Args,
    Action::OffloadKind DeviceOffloadKind) const {
  // MorphOS does not support .init_array.
  CC1Args.push_back("-fno-use-init-array");
  // MorphOS headers have thousands of macro expansions that require GNU
  // extensions.
  CC1Args.push_back("-Wno-gnu-statement-expression-from-macro-expansion");
}
...
void MorphOS::addLibStdCxxIncludePaths(
    ...
    llvm::opt::ArgStringList &CC1Args) const {
  // FIXME: Discover GCC instead of hard-coding the version.
  const std::string GCCLibPath =
      getDriver().SysRoot + "/lib/gcc-lib/ppc-morphos/15.1.0";
  const bool NoIxemul = DriverArgs.hasArg(options::OPT_noixemul);

  if (NoIxemul) {
    addLibStdCXXIncludePaths(GCCLibPath + "/include/c++/ppc-morphos/libnix", "",
                             "", DriverArgs, CC1Args);
  } else {
    addLibStdCXXIncludePaths(GCCLibPath + "/include/c++/ppc-morphos", "", "",
                             DriverArgs, CC1Args);
  }
  addLibStdCXXIncludePaths(GCCLibPath + "/include/c++", "", "", DriverArgs,
                           CC1Args);
}
