...
public:
  ...
  MCInst createBundle(MCContext &Ctx, ArrayRef<MCInst> Instrs,
    ...
    // Set hardware loop end markers. The MC code emitter uses these
    // flags to emit INST_PARSE_LOOP_END (0x8000) parse bits on the
    // appropriate instruction words within the packet.
    if (InnerLoop)
      HexagonMCInstrInfo::setInnerLoop(Bundle);
    if (OuterLoop)
      HexagonMCInstrInfo::setOuterLoop(Bundle);
