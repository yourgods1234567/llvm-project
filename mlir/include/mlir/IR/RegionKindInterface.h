...
/// For a RegionTerminator with num-breaking-regions = N (N > 1), walk up N-1
/// parent op levels from its immediate parent to find the
/// HasBreakingControlFlowOpInterface ancestor that actually consumes the break.
/// Returns a null interface wrapper if the chain is malformed (intermediate op
/// missing, or the target does not implement
