...
static void replaceOpWithRegion(PatternRewriter &rewriter, Operation *op,
  ...
  if (terminator->getNumBreakingControlRegions() < 1) {
    ...
    // Walk backwards so that each op is erased before its predecessor is
    // inspected, avoiding use-after-free on the termination check.
    Operation *cur = &op->getBlock()->back();
    while (cur != op) {
