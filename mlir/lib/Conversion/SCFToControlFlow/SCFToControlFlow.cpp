...
/// become branches to the continuation block and `scf.continue` ops become
/// back-edges to the loop header. Multi-level breaks (num-breaking-regions > 1)
/// are not yet supported and cause the pattern to bail out.
...
LoopOpLowering::matchAndRewrite(LoopOp loopOp, OpAdaptor adaptor,
  ...
  // Check for unsupported multi-level breaks targeting this loop.
  bool hasUnsupportedPredecessors = false;
  visitNestedBreakingControlFlowOps(
      loopOp, [&](Operation *op, int nestedLevel) {
        if (op->getNumBreakingControlRegions() > 1 &&
            nestedLevel == static_cast<int>(op->getNumBreakingControlRegions()))
          hasUnsupportedPredecessors = true;
        return hasUnsupportedPredecessors ? WalkResult::interrupt()
                                          : WalkResult::advance();
      });
  if (hasUnsupportedPredecessors)
    return rewriter.notifyMatchFailure(loopOp,
