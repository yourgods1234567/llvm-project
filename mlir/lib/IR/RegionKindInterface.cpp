...
HasBreakingControlFlowOpInterface mlir::findBreakTarget(Operation *terminator) {
  int depth = terminator->getNumBreakingControlRegions();
  // depth=2: immediate parent is level 1, target is level 2.
  // Walk up depth-1 additional levels from the immediate parent.
  Operation *op = terminator->getParentOp(); // level 1
  for (int i = 1; i < depth; ++i) {
    if (!op)
      return {};
    op = op->getParentOp();
  }
  return dyn_cast_or_null<HasBreakingControlFlowOpInterface>(op);
}
